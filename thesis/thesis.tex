...
\begin{itemize}
    ...
    \item Klucz API Google Gemini\footnote{Można wygenerować darmowy klucz w \href{https://aistudio.google.com}{google ai studio}.}
\end{itemize}
...
Na rys.~\ref{img:login} przedstawiono formularz umożliwiający zarówno rejestrację nowego użytkownika, jak i logowanie do systemu.
...
Na rys.~\ref{img:notes} przedstawiono główny widok notatek użytkownika, umożliwiający tworzenie, przeglądanie, edycję oraz usuwanie notatek.
...
Na rys.~\ref{img:groups} przedstawiono widok grup notatek użytkownika, umożliwiający przeglądanie, tworzenie oraz zarządzanie grupami semantycznymi.
...
Na rys.~\ref{img:search} przedstawiono widok semantycznego wyszukiwania notatek użytkownika, umożliwiający szybkie i inteligentne odnajdywanie treści w systemie.
...
Na (rys.~\ref{img:ask}) przedstawiono panel umożliwiający zadawanie pytań modelowi językowemu. Aby zadać pytanie, użytkownik przechodzi do strony \textbf{Ask} w interfejsie aplikacji, wprowadza treść pytania w odpowiednim polu i zatwierdza je przyciskiem \textbf{Ask Question}. System automatycznie wyszukuje fragmenty notatek powiązanych tematycznie z pytaniem, a następnie generuje odpowiedź przy użyciu modelu językowego, zapewniając spójność i trafność udzielanej informacji oraz dodając cytacje użytych notatek.
